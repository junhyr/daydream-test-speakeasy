...
import { DaydreamTestDefaultError } from "../models/errors/daydreamtestdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | DaydreamTestDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | DaydreamTestDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new DaydreamTestDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new DaydreamTestDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
